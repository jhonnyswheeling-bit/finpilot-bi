...
const App = {
  ...
  chooseDefaults(){
    // metric: prioriza uma coluna claramente "Valor" (ou variação óbvia
    // do nome); só cai na heurística de maior soma absoluta quando
    // nenhuma coluna assim existir entre as numéricas.
    const normalize=s=>String(s).trim().toLowerCase();
    const tokens=s=>normalize(s).split(/\s+/).filter(Boolean);
    const numericCandidateWithToken=(tok)=> state.numericCols.find(c=>tokens(c).includes(tok));

    let best = numericCandidateWithToken("valor");
    if(!best){
      // outras colunas financeiras óbvias, antes de cair na heurística
      const outrosNomes=["montante","total","quantia","amount","preco","preço"];
      for(const nome of outrosNomes){
        best = numericCandidateWithToken(nome);
        if(best) break;
      }
    }
    if(!best){
      // fallback original: numeric col with largest sum of abs values
      let bestSum=-1;
    ...
    // [DIAG TEMPORÁRIO — remover depois de confirmar a causa em produção]
    console.log("[DIAG chooseDefaults] numericCols:", state.numericCols);
    console.log("[DIAG chooseDefaults] state.metricCol escolhido:", state.metricCol);
    if(state.metricCol){
      const somaMetric = state.finalData.reduce((a,r)=>{const n=parseNumberFlexible(r[state.metricCol]); return a+(n||0);},0);
      console.log("[DIAG chooseDefaults] soma de '"+state.metricCol+"':", somaMetric);
    }
    // [FIM DIAG TEMPORÁRIO]
